...
maintenanceRouter.post('/', isAuth, async (req, res) => {
  ...
  try {
    ...
    if (idsOfnotExistingPieces.length > 0) {
      return res.status(401).send('Quantité des piéces non disponible');
    } else {
...
maintenanceRouter.put('/:id', isAuth, async (req, res) => {
  ...
  try {
    ...
    if (idsOfnotExistingPieces.length > 0) {
      return res.status(401).send('Quantitées des piéces non disponible');
    }
